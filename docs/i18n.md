...
| `fr-FR` | `web/src/locales/fr-FR.json` | |
| `zh-CN` | `web/src/locales/zh-CN.json` | Mandarin (Simplified) |
| `hi-IN` | `web/src/locales/hi-IN.json` | |
| `ar-SA` | `web/src/locales/ar-SA.json` | right to left (sets `<html dir="rtl">`) |

The set picks the most spoken languages in the world that were not already
published (English, Portuguese and Spanish are the fallback, `pt-BR` and `es-MX`).

`DEFAULT_LOCALE` selects the language offered to **first time visitors** and must
be one of those values (validated at boot). The administrator can change it
at runtime in Admin > Settings; the value is stored in the `settings` table
(`default_locale`), so all new visitors see the same default. In the default
shared-database mode every cluster node sees it too; in federated mode it reaches
the other nodes when `CLUSTER_SYNC_SETTINGS=true` (see
...
- `document.documentElement.lang` is updated on every change (accessibility and
  correct hyphenation), and `document.documentElement.dir` follows the language:
  `ar-SA` is written right to left, so the whole layout is mirrored. The
  Tailwind classes use logical properties (`ms-`, `pe-`, `start-`, `end-`,
  `text-start`, …) and a few transforms carry an explicit `rtl:` variant; a tiny
  inline script in `web/index.html` applies the stored direction before the first
  paint.
...
A right to left language needs two more steps: add its base code to
`RTL_LANGUAGES` in `web/src/i18n.ts` and to the list in the pre-paint script of
`web/index.html`, then make sure every class it renders is direction agnostic
(logical `ms-`/`me-`/`ps-`/`pe-`/`start-`/`end-` utilities or an `rtl:` variant
for a transform).

When a transform also needs a breakpoint override, scope the **pair** with
`ltr:`/`rtl:` instead of resetting one direction variant with `lg:`. Tailwind
emits the direction variants after the breakpoint ones, so `lg:translate-x-0`
loses to `rtl:translate-x-full` and the element stays off screen on desktop (the
mobile drawer of the sidebar was the case that exposed it).
...
| `notifications` | channel management (SMTP/Webhook/Slack/Discord/Telegram) |
...
Keys must match exactly across every locale file. A quick check:
