...
<template>
  <DialogRoot :open="props.modelValue" @update:open="emit('update:modelValue', $event)">
    <DialogPortal>
      ...
      <DialogContent
        class="fixed start-1/2 top-1/2 z-50 max-h-[92vh] w-[calc(100vw-2rem)] -translate-x-1/2 rtl:translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl border border-border bg-card p-5 shadow-xl"
